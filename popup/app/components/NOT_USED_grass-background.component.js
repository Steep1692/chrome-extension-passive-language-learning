...
// Init and Cleanup ///////////////////////////////////////////
    function init(canvas) {
      ...
      setTimeout(() => {
        currentWinds.push({
          wind: -0.8,
          posX: fieldCanvas?.clientWidth + 10,
          speed: -500 / 1000,
          speedUp: -2 / 1000,
          powerUp: -0.5 / 1000,
        })
...
// Event Listners //////////////////////////////////////
    ...
    function onMouseMove(event) {
      ...
      const inRange = event.clientY > window.innerHeight - fieldCanvas?.clientHeight
      if (inRange) {
    ...
    function onWindowResize() {
      ...
      for (const layer of layers) {
        layer.grassBlades = resizeGrassBlades(layer.grassBlades, layer)
      }
      startAnimationIfNotAlreadyRunning()
    }
...
// Setup Functions ///////////////////////////////////////////
    function setupGrass() {
      for (const layer of layers) {
        layer.grassBlades = resizeGrassBlades(null, layer)
      }
    }

    function resizeGrassBlades(grassBlades = null, layer) {
      const newSize = Math.ceil(fieldCanvas?.clientWidth / layer.pixelPerGrass)
      if (grassBlades == null) {
        grassBlades = {
          length: 0,
...
// Animation ////////////////////////////////////////////////
    ...
    function applyWinds(deltaTime) {
      ...
      for (const wind of currentWinds) {
        ...
        if (
          (wind.speed >= 0 && wind.posX >= fieldCanvas?.clientWidth) ||
...
// Draw On Canvas ///////////////////////////////////////////
    function clearCanvas() {
      // fieldCanvasCtx.clearRect(0, 0, fieldCanvas?.clientWidth, fieldCanvas?.clientHeight);
      fieldCanvasCtx.clearRect(0, 0, fieldCanvas?.clientWidth, 300)
    }
...
customElements.define('pll-grass-background', GrassWebComponent)